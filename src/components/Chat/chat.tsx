...
const Chat: React.FC<MesaIdI> = ({ mesaId }) => {
    ...
    const getDadosMensagens = async () => {
        ...
        try {
            ...
            const dados: MensagemI[] = await response.json(); // Tipando os dados como uma lista de mensagens
            setMensagens(dados);  // Atualiza o estado das mensagens
        } catch (error) {
            console.error("Erro ao buscar mensagens: ", error);
        }
    };

    // Configura o intervalo de 1 segundo para recuperar as mensagens repetidamente
    useEffect(() => {
